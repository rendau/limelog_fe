<template>
  <div class="row items-start q-col-gutter-sm cursor-pointer">
    <div :class="withTime ? 'col-8' : 'col-12'">
      <q-field outlined :label="label" :stack-label="!!date"
               bg-color="white" class="cursor-pointer">
    ...
    <div v-if="withTime" class="col-4">
      <q-field outlined :label="timeLabel" :stack-label="!!time"
               bg-color="white" class="cursor-pointer">
        <template v-slot:control>
    ...
    methods: {
      ...
      emitInput () {
        ...
        if (this.date) {
          ...
          value = moment(this.date + ' ' + time, 'YYYY-MM-DD HH:mm').toJSON()
        }
